...
# Edited Aws0mus
# Now the RTU device receives a list of dictionaries as protocols to support several protocols in a single device
# Methods override in RTU class
...
# New RTU supports multiple protocols
class RTU(Device):
    ...
    """
    # Overrided
    def __init__(self, name, protocols, state, disk={}, memory={}):

        self._validate_inputs(name, protocols, state, disk, memory)
    ...
    # Overrided
    def _validate_inputs(self, name, protocols, state, disk, memory):
        ...
        if len(protocols) < 1:
            raise TypeError('No protocol selected for the RTU')
    ...
    def _init_protocols(self):
        ...
        if self.protocols is None:
            ...
            for protocol in self.protocols:
                name = protocol['name']
                if name == 'enip':
                    self._protocols.append(EnipProtocol(protocol))
                elif name == 'modbus':
                    self._protocols.append(ModbusProtocol(protocol))
                else:
                    print('ERROR: %s protocol not supported.' % self.protocol)
                i += 1

    # Overrided
    def send(self, what, value, address, protocol_name, **kwargs):
    ...
    # Overrided
    def receive(self, what, address, protocol, **kwargs):
    ...
    def _stop(self):
        ...
        for protocol in self.protocols:
            if protocol['mode'] > 0:
                self._protocol[i]._server_subprocess.kill()
            i += 1
